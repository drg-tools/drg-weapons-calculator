...
public class WeaponStatsGenerator {
	...
	public WeaponStatsGenerator(Weapon testingWeapon) {
		...
		headers = new String[] {"Mods/OC", "Burst DPS", "Sustained DPS", "Additional Target DPS", "Max Multi-Target Dmg", "Max Num Targets", "Firing Duration"};
	...
	public void runTest(boolean printStatsToConsole, boolean exportStatsToCSV) {
		...
		*/
					...
					for (int t3 = -1; t3 < weaponToTest.getModsAtTier(3).length; t3++) {
						...
						for (int t2 = -1; t2 < weaponToTest.getModsAtTier(2).length; t2++) {
							...
							for (int t1 = -1; t1 < weaponToTest.getModsAtTier(1).length; t1++) {
								...
								if (weaponToTest.calculateBurstDPS() > bestBurstDPS.calculateBurstDPS()) {
									bestBurstDPS = weaponToTest.clone();
								}
								if (weaponToTest.calculateSustainedDPS() > bestSustainedDPS.calculateSustainedDPS()) {
		...
		if (printStatsToConsole) {
			...
			System.out.println("	Best Burst DPS: " + bestBurstDPS.getCombination() + " at " + bestBurstDPS.calculateBurstDPS()  + " DPS");
			System.out.println("	Best Sustained DPS: " + bestSustainedDPS.getCombination() + " at " + bestSustainedDPS.calculateSustainedDPS() + " DPS");
	...
	private void calculateStatsAndPrint(boolean console, boolean csv) {
		...
		// Single-target calculations
		double burstDPS = weaponToTest.calculateBurstDPS();
		double sustainedDPS = weaponToTest.calculateSustainedDPS();
		
		// Multi-target calculations
		double addTargetDPS = weaponToTest.calculateAdditionalTargetDPS();
		double maxDamage = weaponToTest.calculateMaxMultiTargetDamage();
		
		// Non-damage calculations
		int maxNumTargets = weaponToTest.calculateMaxNumTargets();
		double duration = weaponToTest.calculateFiringDuration();
		
		if (console) {
			printStatsToConsole(combination, burstDPS, sustainedDPS, addTargetDPS, maxDamage, maxNumTargets, duration);
		}
		if (csv) {
			printStatsToCSV(combination, burstDPS, sustainedDPS, addTargetDPS, maxDamage, maxNumTargets, duration);
		}
	}
	
	private void printStatsToConsole(String combination, double burst, double sustained, double add, double maxDmg, int numTargets, double duration) {
		String format = "%s,\t\t%f,\t%f,\t%f,\t\t%f,\t\t%d,\t\t\t%f\n";
		System.out.printf(format, combination, burst, sustained, add, maxDmg, numTargets, duration);
	}
	
	private void printStatsToCSV(String combination, double burst, double sustained, double add, double maxDmg, int numTargets, double duration) {
		String format = "%s, %f, %f, %f, %f, %d, %f,\n";
		csvLinesToWrite.add((String.format(format, combination, burst, sustained, add, maxDmg, numTargets, duration)));
	}
